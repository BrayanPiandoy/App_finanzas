...
class NovAhorrosFragment : Fragment() {
    private val categorias = listOf("Cuenta de ahorros", "Fondos de inversión", "Plan de pensiones", "Cuenta de jubilación", "Inversiones a largo plazo", "Inversiones a corto plazo", "Otros")
    ...
    private fun saveEgreso() {
        ...
        if (user != null) {
            val uid = user.uid
            val egresoRef = database.getReference("users").child(uid).child("ahorros").push()
